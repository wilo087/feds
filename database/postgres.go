...
func (r *PostgresRepository) ListFeeds(ctx context.Context) ([]*models.Feed, error) {
	rows, err := r.db.QueryContext(ctx, "SELECT id, title, description, created_at, updated_at FROM feeds")
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		feed := &models.Feed{}
		if err := rows.Scan(&feed.ID, &feed.Title, &feed.Description, &feed.CreatedAt, &feed.UpdatedAt); err != nil {
			return nil, err
		}
